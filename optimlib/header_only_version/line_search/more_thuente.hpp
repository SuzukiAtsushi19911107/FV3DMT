...
internal::line_search_mt(
    ...
    void* opt_data)
{
    const size_t iter_max = 10;

    const fp_t step_min = 0.001;
    const fp_t step_max = 10.0;
    const fp_t xtol = 1E-04;

    // Wolfe parameters
    const fp_t wolfe_cons_1 = (wolfe_cons_1_inp) ? *wolfe_cons_1_inp : 1E-04; // tolerance on the Armijo sufficient decrease condition; sometimes labelled 'mu'.
    const fp_t wolfe_cons_2 = (wolfe_cons_2_inp) ? *wolfe_cons_2_inp : 0.9;  // tolerance on the curvature condition; sometimes labelled 'eta'.
