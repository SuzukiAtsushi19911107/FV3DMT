    ...
    algo_settings_t* settings_inp)
{
	...
	fp_t step = settings.bfgs_settings.step; //add 20220213 from 1.0
    ...
    while (grad_err > grad_err_tol && rel_sol_change > rel_sol_change_tol && iter < iter_max) {
        ...
        line_search_mt(step, x_p, grad_p, d, &wolfe_cons_1, &wolfe_cons_2, box_objfn, opt_data);
		//step size follows previous one for stability. revised 20220213
		std::cout << "Line Search Step:" << step << std::endl;
