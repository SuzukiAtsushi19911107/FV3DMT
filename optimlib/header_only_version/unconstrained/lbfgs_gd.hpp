...
#ifndef _optim_lbfgs_gd_HPP
#define _optim_lbfgs_gd_HPP
...
bool
lbfgs_gd(ColVec_t& init_out_vals, 
...
bool
lbfgs_gd_impl(ColVec_t& init_out_vals, 
           std::function<fp_t (const ColVec_t& vals_inp, ColVec_t* grad_out, void* opt_data)> opt_objfn, 
           void* opt_data, 
    ...
    algo_settings_t* settings_inp)
{
    ...
    const fp_t wolfe_cons_1 = settings.lbfgs_gd_settings.wolfe_cons_1; // line search tuning parameters
    const fp_t wolfe_cons_2 = settings.lbfgs_gd_settings.wolfe_cons_2;

    const size_t par_M = std::max(static_cast<size_t>(2), settings.lbfgs_gd_settings.par_M); // how many previous iterations to use when updating the Hessian
	...
	const fp_t initStep = settings.lbfgs_gd_settings.step;

	const fp_t eta = settings.lbfgs_gd_settings.eta;
	...
	line_search_mt(initStep, x_p, grad_p, d, &wolfe_cons_1, &wolfe_cons_2, box_objfn, opt_data);
    ...
    while (grad_err > grad_err_tol && rel_sol_change > rel_sol_change_tol && iter < iter_max) {
		...
		//fp_t direcSize = d.norm();
		//d = d -eta * grad;
		//d = d / d.norm() * direcSize; //set the same norm to the vector of the original lbfgs method.
		//
		//d = -grad; //test

		line_search_mt(step, x_p, grad_p, d, &wolfe_cons_1, &wolfe_cons_2, box_objfn, opt_data);
		...
		ColVec_t d_p = gd_update(x, grad, grad_p, d_gd, box_objfn, opt_data, iter,
		...
		OPTIM_GD_TRACE(iter , grad_err, rel_sol_change, x, d, grad_p, adam_vec_m, adam_vec_v)
    }
...
lbfgs_gd(ColVec_t& init_out_vals, 
             ...
             void* opt_data)
{
    return internal::lbfgs_gd_impl(init_out_vals,opt_objfn,opt_data,nullptr);
}
...
lbfgs_gd(ColVec_t& init_out_vals, 
             ...
             algo_settings_t& settings)
{
    return internal::lbfgs_gd_impl(init_out_vals,opt_objfn,opt_data,&settings);
}
