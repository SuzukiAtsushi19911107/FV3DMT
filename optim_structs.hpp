...
struct gd_settings_t
{
	...
	bool isRestartAdam = false; //add 
	double maxStepSize = 1; //add

	double loosenFactor = 1.0; //add
	double decreaseFactor = 0.0;
...
struct lbfgs_settings_t
{
    ...
    fp_t wolfe_cons_2 = 0.90;  // line search tuning parameter
	fp_t step = 1.0;
	fp_t loosenFactor = 1.1; //add below for back tracking
	fp_t decreaseFactor = 0.5;
	fp_t minStep = 0.0001;
    size_t maxIterationLineSearch = 10;
};

struct lbfgs_gd_settings_t //add
{
	size_t par_M = 10;
	fp_t wolfe_cons_1 = 1E-03; // line search tuning parameter
	fp_t wolfe_cons_2 = 0.90;  // line search tuning parameter
...
struct algo_settings_t
{
	...
	fp_t objFuncVal;
	fp_t rel_resis_change_tol = 1e-4;
	ColVec_t resisVec;
	ColVec_t resisVec_p;

    ColVec_t* useImpedanceDataVec;
    ColVec_t* useTipperDataVec;
	...
	bool isFinishOptimize = false; //add to control convergence from outer function
	size_t iteration = 0;
	...
	//add to print infomation in optimize function.
	...
	// L-BFGS
	lbfgs_gd_settings_t lbfgs_gd_settings;
