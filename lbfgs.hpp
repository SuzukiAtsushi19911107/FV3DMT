    ...
    algo_settings_t* settings_inp)
{
	...
	double objFuncVal = 1.0;
	fp_t objFuncVal_p = 1e30; //add

    double step = 1.0;



    //line_search_bt(1.0, x_p, grad_p, d, &wolfe_cons_1, &wolfe_cons_2, settings_inp->lbfgs_settings.minStep, settings_inp->lbfgs_settings.decreaseFactor, box_objfn, opt_data, settings_inp, &success);
    ...
    fp_t eps_cautiousUpdate = 0.01;
    fp_t alpha_cautiousUpdate = 2;

    ColVec_t relResisMaxChangeVec(averageIter);

	while  (grad_err > grad_err_tol && rel_sol_change > rel_sol_change_tol && iter < iter_max &&
		std::abs(objFuncVal - objFuncVal_p) / (objFuncVal_p + OPTIM_FPN_SMALL_NUMBER)>settings.rel_objfn_change_tol
		&& (iter< averageIter || relChangeResisMax > settings_inp->rel_resis_change_tol) && success) {
        ...
        d = - lbfgs_recur(grad, s_mat, y_mat, std::min(numVec, par_M));

        double step_p = step;
        step = line_search_mt(step, x_p, grad_p, d, &wolfe_cons_1, &wolfe_cons_2, box_objfn, opt_data);
        //step = line_search_bt(step, x_p, grad_p, d, &wolfe_cons_1, &wolfe_cons_2, settings_inp->lbfgs_settings.minStep, settings_inp->lbfgs_settings.decreaseFactor, box_objfn, opt_data, settings_inp, &success);

     /*   if (success) {
            step = settings_inp->lbfgs_settings.loosenFactor * step;
        }*/
        std::cout << "STEP SIZE IN LBFGS:" << step << std::endl;
        // if ||gradient(x_p)|| > tolerance, then continue
        std::cout << "step_p:" << step_p << " step:" << step << std::endl;
        if (std::abs(step_p- step) <=  OPTIM_FPN_SMALL_NUMBER) {
            step *= settings_inp->lbfgs_settings.loosenFactor;
        }
        else {
            step = 1.0;
        }
        ...
        x = x_p;
        grad = grad_p;
        int vecLen = std::min(numVec, par_M);
        //if (BMO_MATOPS_L1NORM(grad_p) > 1.0) { //following D.-H. Li and M. Fukushima
        //    alpha_cautiousUpdate = 0.01;
        //}
        //else {
        //    alpha_cautiousUpdate = 3.0;
        //}
        //alpha_cautiousUpdate = 1.0;

        if (BMO_MATOPS_DOT_PROD(y_mat.col(vecLen - 1), s_mat.col(vecLen - 1))/ BMO_MATOPS_DOT_PROD(s_mat.col(vecLen - 1), s_mat.col(vecLen - 1))>=
            eps_cautiousUpdate* std::pow(BMO_MATOPS_L1NORM(grad_p),alpha_cautiousUpdate)) {
            ...
            numVec--;
            std::cout << "Update Hessian in L-BFGS has been Skipped following Cautious Update Scheme!!!!" << std::endl;
        }// cautious update following D.-H. Li and M. Fukushima. On the global convergence of the BFGS method for nonconvex unconstrained
		...
		objFuncVal = settings_inp->objFuncVal;
        ...
        if (iter - 1 < averageIter) {
            relResisMaxChangeVec(iter - 1) = relChangeResisMax;
        }
        else {
            for (int i = 0; i < averageIter - 1; i++) {
                relResisMaxChangeVec(i) = relResisMaxChangeVec(i + 1);
            }
            relResisMaxChangeVec(averageIter - 1) = relChangeResisMax;

            //calc Average
            relChangeResisMax = 0.0;
            for (int i = 0; i < averageIter; i++) {
                relChangeResisMax += relResisMaxChangeVec(i);
            }
            relChangeResisMax = relChangeResisMax / averageIter;
        }
